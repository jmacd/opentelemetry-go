...
//  1. If a current upstream context is set
//     a. If a current Span is set with the same trace_id, use the Span's
//        SpanContext as parent.
//     b. If the current Span belongs to a different trace_id, use the upstream
//        context are parent.
//  2. If a current span is set and upstream context is not set, use its context.
//  3. If neither upstream nor span context is set, a new root.
