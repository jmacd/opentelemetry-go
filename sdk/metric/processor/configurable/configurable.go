...
func (ci *Processor) newInstDefault() [][]*prototype {
	...
	addDef(metric.CounterKind, aggregation.SumKind, ci.sumAggregator)
	addDef(metric.UpDownCounterKind, aggregation.SumKind, ci.sumAggregator)
	addDef(metric.SumObserverKind, aggregation.SumKind, ci.sumAggregator)
	addDef(metric.UpDownSumObserverKind, aggregation.SumKind, ci.sumAggregator)
	addDef(metric.ValueRecorderKind, aggregation.MinMaxSumCountKind, ci.minmaxsumcountAggregator)
	addDef(metric.ValueObserverKind, aggregation.MinMaxSumCountKind, ci.minmaxsumcountAggregator)

	return id
}

func New(cfg Config, kind export.ExporterKind) (*Processor, error) {
...
func (ci *Processor) sumAggregator(_ *metric.Descriptor) export.Aggregator {
	return sum.New()
}

func (ci *Processor) minmaxsumcountAggregator(desc *metric.Descriptor) export.Aggregator {
	return minmaxsumcount.New(desc)
}

func (ci *Processor) histogramAggregator(desc *metric.Descriptor) export.Aggregator {
	return histogram.New(desc, nil)
}

func (ci *Processor) lastvalueAggregator(desc *metric.Descriptor) export.Aggregator {
	return lastvalue.New()
}

func (ci *Processor) sketchAggregator(desc *metric.Descriptor) export.Aggregator {
	return ddsketch.New(desc, ddsketch.NewDefaultConfig())
}

func (ci *Processor) arrayAggregator(desc *metric.Descriptor) export.Aggregator {
	return array.New()
}

func (ci *Processor) aggregationFor(desc *metric.Descriptor) []*prototype {
...
func (ci *Processor) AggregatorFor(desc *metric.Descriptor) export.Aggregator {
...
func (ci *Processor) Process(record export.Accumulation) error {
...
// func (b *Processor) FinishedCollection() {
// 	if !b.stateful {
